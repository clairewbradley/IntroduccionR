...
zika2 <- read.csv("zika_con_pruebas_practica.csv")
...
zika2$lugar[zika2$hotspot == 1] # Quetzaltenango y Zacapa
...
zika_ac <- subset(zika2, tipoPrueba == "Anticuerpo")

  #c: Copie la estructura de #9 para visualizar los datos en un gráfico, cambie el título y el marbete de axis
barplot(zika_ac$casos)
barplot(zika_ac$casos, main = "Zika por Región, Prueba Anticuerpo", names.arg = zika_ac$lugar, las = 2) 
